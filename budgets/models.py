...
class BudgetAbstract(models.Model):
    ...
    def _update_spent(self):
        ...
        spent = Record.objects.filter(
            user=self.user, transaction_type='EXP', created_at__gte=start_day
        )
...
class Budget(BudgetAbstract):
    ...
    name = models.CharField(
        max_length=100, default=_(u'Monthly budget'), verbose_name=_(u'Budget name')
    )
...
class YearBudget(BudgetAbstract):
    name = models.CharField(
        max_length=100, default=_(u'Year budget'), verbose_name=_(u'Budget name')
    )
